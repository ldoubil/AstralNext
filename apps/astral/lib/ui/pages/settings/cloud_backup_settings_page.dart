...
class _CloudBackupSettingsPageState extends State<CloudBackupSettingsPage> {
  ...
  bool? _connectionStatus; // null = 未测试, true = 成功, false = 失败
  ...
  Future<void> _confirmRestore(RemoteBackupEntry entry) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('恢复成功，部分设置可能需要重启应用后生效'),
            backgroundColor: Colors.green,
          ),
        );
      }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '将当前实例配置、应用设置和面板布局打包备份到 WebDAV，或从云端恢复。',
                  style: TextStyle(
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    _formatDate(
                        entry.lastModified ?? DateTime.fromMillisecondsSinceEpoch(0)),
                    style: TextStyle(
                      color: colorScheme.onSurface,
                      fontSize: 14,
